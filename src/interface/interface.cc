...
void Interface::InitFuncMenus() {
  ...
  m_funcs[MenuFuncs::kMainFuncMenu] = {
      ...
      std::bind(&Interface::RunMenu, this,
                std::ref(m_funcs[MenuFuncs::kRabinKarpMenu]),
                MenuItem::kLoadMenu),
      std::bind(&Interface::RunMenu, this,
                std::ref(m_funcs[MenuFuncs::kNWAlgorithmMenu]),
                MenuItem::kLoadMenu),
      std::bind(&Interface::RunMenu, this,
                std::ref(m_funcs[MenuFuncs::kRegexMenu]), MenuItem::kLoadMenu),
      std::bind(&Interface::RunMenu, this,
                std::ref(m_funcs[MenuFuncs::kSimilarMenu]),
                MenuItem::kLoadMenu),
      std::bind(&Interface::RunMenu, this,
                std::ref(m_funcs[MenuFuncs::kWindowMenu]),
                MenuItem::kLoadMenu)};

  m_funcs[MenuFuncs::kRabinKarpMenu] = {
      std::bind(&Interface::Exit, this), [this]() -> const bool {
        return RunProcessFile2arg(
            std::bind(&DNA_Analyzer::RabinKarpAlgorithm, std::ref(m_analyzer),
                      std::placeholders::_1, std::placeholders::_2));
      }};

  m_funcs[MenuFuncs::kNWAlgorithmMenu] = {
      std::bind(&Interface::Exit, this), [this]() -> const bool {
        return RunProcessFile1arg(std::bind(&DNA_Analyzer::NWAlgorithm,
                                            std::ref(m_analyzer),
                                            std::placeholders::_1));
      }};
  ...
  m_funcs[MenuFuncs::kRegexMenu] = {
      std::bind(&Interface::Exit, this), [this]() -> const bool {
        return RunProcessFile1arg(std::bind(&DNA_Analyzer::RegexAlgorithm,
                                            std::ref(m_analyzer),
                                            std::placeholders::_1));
      }};
  ...
  m_funcs[MenuFuncs::kSimilarMenu] = {
      std::bind(&Interface::Exit, this), [this]() -> const bool {
        return RunProcessFile1arg(std::bind(&DNA_Analyzer::KSimilarAlgorithm,
                                            std::ref(m_analyzer),
                                            std::placeholders::_1));
      }};
  ...
  m_funcs[MenuFuncs::kWindowMenu] = {
      std::bind(&Interface::Exit, this), [this]() -> const bool {
        return RunProcessFile1arg(std::bind(&DNA_Analyzer::WindowAlgorithm,
                                            std::ref(m_analyzer),
                                            std::placeholders::_1));
      }};
}

const bool Interface::RunProcessFile1arg(
...
const bool Interface::RunProcessFile2arg(
